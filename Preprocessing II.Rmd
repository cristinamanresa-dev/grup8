...
### 4.1 Generar Data amb NA's, nosaltres ja els tenim, es poden veure
```{r}
colSums(is.na(train))
```
...
#train <- dplyr::mutate_if(train, is.character, as.factor)
```

segueix sense funcionar
```{r}
#no ens funciona perque ens diu que no es pot calcular la matriu de covariancies 
...
No surt en compte perque tenim el maeix nombre de missings a totes les variables
```{r}
library(visdat)
library(ggplot2)
library(naniar)
...
#no funciona i tarda molt a carregar
# ggplot(train, aes(x = speechiness ,y = loudness)) + 
 # geom_miss_point() +
 # facet_wrap(~ song_duration_ms)
```
...
```
veiem que totes les variables tenen el mateix nombre de NA

#### Detecció de NA's en la bbdd
aixo ho podem anar fent provant amb diferents variables aviam que observem
```{r}
aq_shadow <- bind_shadow(train)

train %>%
  bind_shadow() %>%
  group_by(song_duration_ms) %>%
  summarise_at(.vars = "speechiness",
               .funs = c("mean", "sd", "var", "min", "max"),
               na.rm = TRUE)

ggplot(train,
       aes(x = loudness,
           colour = song_duration_ms)) + 
  geom_density()
```
...
train[, c("imputed_speechiness", "imputed_speechiness2")] <- NULL
```
veiem que les mes similars son speechiness i la imputed2, pero que voldir?

no sarriba a veure la imputed 2
