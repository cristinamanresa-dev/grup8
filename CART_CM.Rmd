...
prediccions_tree <- data.frame(
  id = test_original$ID,
  song_popularity = round(predict(arbol_regresion, test_original))
)
...
```{r}
pred_nou <- predict(caret.rpart, newdata = test_original)
pred_nou <- round(pred_nou)

test_original$pred_song_popularity <- pred_nou
